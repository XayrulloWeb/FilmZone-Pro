...
import { AnimatePresence, motion } from 'framer-motion'; // <--- ИСПРАВЛЕНИЕ ТУТ
...
const AuthModal = ({ isOpen, onClose }) => {
  ...
  // Если модалка закрыта, ничего не рендерим (но AnimatePresence требует наличия компонента в дереве для выхода)
  // Поэтому условие !isOpen лучше обрабатывать внутри AnimatePresence или выше, 
  // но в твоем коде логика выхода через exit анимацию, поэтому рендерим null только если не isOpen И анимация завершилась.
  // Но для простоты оставим как есть, AnimatePresence сам разберется с isOpen через условие внутри.
    ...
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
            ...
            className="relative w-full max-w-md bg-surface border border-white/10 rounded-2xl shadow-2xl overflow-hidden z-10"
            >
            {/* Header */}
